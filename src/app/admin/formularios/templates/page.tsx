...
export const metadata: Metadata = {
  title: 'Formulários — Oliveira Imóveis',
}

export default async function FormulariosTemplatesPage() {
  const supabase = await createClient()

  const [{ data: fieldsData }, { data: templatesData }, { data: usedData }] =
    await Promise.all([
      supabase.from('form_fields').select('*').order('created_at', { ascending: true }),
      supabase.from('form_templates').select('*').order('sort_order', { ascending: true }),
      supabase.from('form_template_fields').select('form_field_id'),
    ])
  ...
  return (
    <div className={styles.page}>
      <FormulariosEditor
        ...
        title="Formulários"
        subtitle="Gerencie os modelos de formulário utilizados no portal."
      />
    </div>
  )
}
