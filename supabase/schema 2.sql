...
-- LLM-generated output per form response
create table public.llm_results (
  id                uuid primary key default gen_random_uuid(),
  form_response_id  uuid not null references public.form_responses(id) on delete cascade,
  result_type       text,   -- e.g. 'budget_table', 'market_report', 'area_profile'
  content           jsonb,  -- structured output from the LLM
  created_at        timestamptz default now()
);
...
-- llm_results: same scoping as field_responses
create policy "Users can view own llm results"
  on public.llm_results for select
  using (
    exists (
      select 1 from public.form_responses r
      where r.id = form_response_id and r.user_id = auth.uid()
...
create policy "Admins can view all llm results"
  on public.llm_results for select using (public.is_admin());
